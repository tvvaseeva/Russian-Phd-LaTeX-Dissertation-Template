...
\node[left,scale=1,font=\small] at (-1.4,6){\large{ВРЕМЯ}};
\node[left,scale=1,font=\small] at (-0.7,5.5){\large{ВЫПОЛНЕНИЯ}};
\node[left,scale=1,font=\small] at (-2,5){\large{[s]}};	
...
%легенда
\draw[ultra thick, red] (1, 6.3) -- (1.5, 6.3) node[right, black, font=\small] {-- \large{FFTW ( with FFTW MEASURE}};
\node[right, black, font=\small] at (1.8, 5.8){\large{and FFTW ESTIMATE)}};
